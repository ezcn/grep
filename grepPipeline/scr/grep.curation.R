...
myd %>% select(index_x,sample, gene_symbol) %>% distinct() %>% group_by( sample, gene_symbol) %>% tally() %>% filter(n>1) %>% write.table(paste(code, '.multiplevariants_pergene.tsv'), sep="\t", quote=F, row.names=F, col.names=T)
