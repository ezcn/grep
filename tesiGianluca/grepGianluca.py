...
def getInfoFromVepLocally (jsonWithVEPannotations):
	...
	with open(jsonWithVEPannotations, 'r') as f:
		for line in f:
			...
			locusdata=info['input'].split(); altAlleles=locusdata[4].split(","); mychr=locusdata[0]; mypos=locusdata[1]; myref=locusdata[3]; myalt=locusdata[4];  mygen=[x.split(":")[0]  for x in locusdata[9:]]
